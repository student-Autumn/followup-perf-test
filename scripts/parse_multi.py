...
files = [
    r'C:\Users\86139\Downloads\测试场景2 150并发.html',
    r'C:\Users\86139\Downloads\测试场景2 100-200.html',
    r'C:\ceshi\练习3\场景2稳定性5小时.html',
]
...
for path in files:
    ...
    if history:
        ...
        for h in history:
            ...
            if rps > peak_rps:
                peak_rps = rps
                peak_rps_time = get_time(h, 'current_rps')
            users = get_val(h, 'user_count')
            if users > peak_users:
                peak_users = users
                peak_users_time = get_time(h, 'user_count')
            rt = get_val(h, 'total_avg_response_time')
            if rt > peak_rt:
                peak_rt = rt
                peak_rt_time = get_time(h, 'total_avg_response_time')
            fail = get_val(h, 'current_fail_per_sec')
            if fail > peak_fail:
                peak_fail = fail
                peak_fail_time = get_time(h, 'current_fail_per_sec')
